#!/usr/bin/env bash
# mock-mv local runner — Linux, macOS, Windows Git Bash/MSYS.
...
PORT="${PORT:-8081}"
PID_FILE="${PID_DIR}/mock-mv.pid"
LOG_FILE="${LOG_DIR}/mock-mv.log"
MODULE="mock-mv"
...
usage() {
  cat <<EOF
Local mock-mv (${PROFILE} profile, :${PORT} ${CONTEXT})
...
print_endpoints() {
  echo
  echo "mock-mv  profile=${PROFILE}  port=${PORT}  context=${CONTEXT}"
...
find_boot_jar() {
  ...
  # Prefer spring-boot finalName (mock-mv.jar) when present
  if [[ -f "${target}/${MODULE}.jar" ]] && [[ "$(wc -c < "${target}/${MODULE}.jar")" -ge 1048576 ]]; then
    echo "${target}/${MODULE}.jar"
    return 0
  fi
...
cmd_start() {
  ...
  check_prereqs
  if is_running; then
    echo "mock-mv already running (pid $(cat "$PID_FILE")) on port ${PORT}"
    print_endpoints
    return 0
  fi
  rm -f "$LOG_FILE"
  local jar
  jar="$(find_boot_jar)"
  echo "==> starting mock-mv from $(basename "$jar")"
...
wait_ready() {
  ...
  while [[ "$elapsed" -lt "$timeout" ]]; do
    ...
    if grep -q "Started ProxyMvApplication" "$LOG_FILE" 2>/dev/null; then
...
cmd_stop() {
  ...
  local pid
  pid="$(cat "$PID_FILE")"
  echo "==> stopping mock-mv (${pid}) on port ${PORT}"
...
cmd_smoke() {
  ...
  local timeout="${1:-60}"
  local elapsed=0
  echo "==> waiting for mock-mv (up to ${timeout}s)"
  while [[ "$elapsed" -lt "$timeout" ]]; do
...
cmd_docker() {
  cmd_init
  need_cmd docker
  docker rm -f mock-mv >/dev/null 2>&1 || true
  echo "==> docker build ${IMAGE}"
  docker build -t "${IMAGE}" "${MODULE_DIR}"
  print_endpoints
  # shellcheck disable=SC2046
  exec docker run --rm -p "${PORT}:8081" --name mock-mv \
    $(docker_host_args) \
