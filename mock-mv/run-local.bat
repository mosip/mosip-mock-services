...
REM Standalone Windows cmd runner for mock-mv (no .ps1).
...
set "PID_FILE=%PID_DIR%\mock-mv.pid"
set "LOG_FILE=%LOG_DIR%\mock-mv.log"
set "ERR_FILE=%LOG_DIR%\mock-mv.err.log"
set "MODULE=mock-mv"

if not defined PORT set "PORT=8081"
if not defined SPRING_PROFILES_ACTIVE set "SPRING_PROFILES_ACTIVE=local"
if not defined IMAGE set "IMAGE=mock-mv"
if not defined loader_path_env set "loader_path_env=."
set "CONTEXT=/v1/mockmv"
...
:usage
echo Local mock-mv ^(%SPRING_PROFILES_ACTIVE% profile, :%PORT% %CONTEXT%^)
echo.
...
:print_endpoints
echo.
echo mock-mv  profile=%SPRING_PROFILES_ACTIVE%  port=%PORT%  context=%CONTEXT%
...
:find_jar
set "BOOT_JAR="
if exist "%MODULE_DIR%\target\%MODULE%.jar" (
  for %%S in ("%MODULE_DIR%\target\%MODULE%.jar") do if %%~zS GEQ 1048576 set "BOOT_JAR=%%~fS"
)
if defined BOOT_JAR goto :find_jar_done
...
if not errorlevel 1 (
  ...
  echo mock-mv already running ^(pid !OLD_PID!^) on port %PORT%
  call :print_endpoints
  exit /b 0
)
call :port_in_use
if not errorlevel 1 (
  call :pid_on_port
  echo mock-mv already listening on port %PORT% ^(pid !PORT_PID!^)
...
echo ==^> starting mock-mv from %BOOT_JAR%
...
set "LAUNCH_PS1=%LOCAL_DIR%\launch-mock-mv.ps1"
...
if not errorlevel 1 (
  if exist "%LOG_FILE%" (
    findstr /C:"Started ProxyMvApplication" "%LOG_FILE%" >nul 2>&1
...
if not defined STOP_PID (
  echo mock-mv is not running
  exit /b 0
)
echo ==^> stopping mock-mv ^(%STOP_PID%^) on port %PORT%
...
:smoke
call :ensure_dirs
echo ==^> waiting for mock-mv ^(up to 60s^)
set /a ELAPSED=0
:smoke_loop
...
docker rm -f mock-mv >nul 2>&1
echo ==^> docker build %IMAGE%
docker build -t %IMAGE% "%MODULE_DIR%"
if errorlevel 1 exit /b 1
call :print_endpoints
docker run --rm -p %PORT%:8081 --name mock-mv -e active_profile_env=%SPRING_PROFILES_ACTIVE% -e spring_config_url_env=%SPRING_CLOUD_CONFIG_URI% -e spring_config_label_env=%SPRING_CLOUD_CONFIG_LABEL% -e JDK_JAVA_OPTIONS=%JDK_JAVA_OPTIONS% %IMAGE%
exit /b %ERRORLEVEL%
