#!/usr/bin/env bash
# MockMDS Auth local runner — Linux, macOS, Windows Git Bash/MSYS.
# Windows cmd: use run-local-auth.bat
#
#   ./run-local-auth.sh init | start | smoke | stop | test | all
...
PID_FILE="${PID_DIR}/mock-mds-auth.pid"
PORT_FILE="${PID_DIR}/mock-mds-auth.port"
LOG_FILE="${LOG_DIR}/mock-mds-auth.log"
MODULE="mock-mds"
SERVICE="mock-mds-auth"
PURPOSE="Auth"
...
usage() {
  cat <<EOF
Local MockMDS — Auth (WSQ), ports ${MIN_PORT}–${MAX_PORT}

  Linux / macOS / Git Bash:
    ./run-local-auth.sh init | start | smoke | stop | test | all

  Windows cmd:
    run-local-auth.bat init | start | smoke | stop | test | all
...
print_endpoints() {
  ...
  echo
  echo "${SERVICE}  purpose=${PURPOSE}  image=${IMAGE_TYPE}  port=${port}"
...
cmd_start() {
  ...
  java_args+=(
    ...
    "mosip.mock.sbi.biometric.image.type=${IMAGE_TYPE}"
  )
  (
    cd "$TARGET_DIR"
