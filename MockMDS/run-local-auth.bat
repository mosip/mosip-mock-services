...
REM Standalone Windows cmd runner for MockMDS Auth (no .ps1).
REM Linux / macOS / Git Bash: use run-local-auth.sh
REM
REM   run-local-auth.bat init | start | smoke | stop | test | all
...
set "PID_FILE=%PID_DIR%\mock-mds-auth.pid"
set "PORT_FILE=%PID_DIR%\mock-mds-auth.port"
set "LOG_FILE=%LOG_DIR%\mock-mds-auth.log"
set "ERR_FILE=%LOG_DIR%\mock-mds-auth.err.log"
set "MODULE=mock-mds"
set "SERVICE=mock-mds-auth"
set "PURPOSE=Auth"
...
:usage
echo Local MockMDS - Auth ^(WSQ^), ports %MIN_PORT%-%MAX_PORT%
echo.
echo   run-local-auth.bat init     package this module
echo   run-local-auth.bat start    run from target/ and wait until SBI is ready
echo   run-local-auth.bat smoke    POST /admin/status on the bound port
echo   run-local-auth.bat stop
echo   run-local-auth.bat test     Maven tests
echo   run-local-auth.bat all      init + test + start + smoke
echo.
...
echo.
echo %SERVICE%  purpose=%PURPOSE%  image=%IMAGE_TYPE%  port=!DISP_PORT!
...
set "LAUNCH_PS1=%LOCAL_DIR%\launch-mock-mds-auth.ps1"
REM Match original argv (quoted Biometric Device + WSQ). Extra quotes needed for Start-Process.
...
>> "%LAUNCH_PS1%" echo $argList.Add^('mosip.mock.sbi.biometric.image.type=%IMAGE_TYPE%'^)
...
set "HTTP_CODE_FILE=%LOCAL_DIR%\http-code-auth.txt"
if "%HTTP_JSON_FILE%"=="" (
...
set /a ELAPSED=0
set "SMOKE_JSON=%LOCAL_DIR%\smoke-status-auth.json"
