#!/usr/bin/env bash
# mock-abis local runner — Linux, macOS, Windows Git Bash/MSYS.
...
PORT="${PORT:-8081}"
PID_FILE="${PID_DIR}/mock-abis.pid"
LOG_FILE="${LOG_DIR}/mock-abis.log"
MODULE="mock-abis"
...
usage() {
  cat <<EOF
Local mock-abis (${PROFILE} profile, :${PORT} ${CONTEXT})
...
print_endpoints() {
  echo
  echo "mock-abis  profile=${PROFILE}  port=${PORT}  context=${CONTEXT}"
...
cmd_init() {
  check_prereqs
  local sample="${MODULE_DIR}/src/main/resources/registration-processor-abis-sample.json"
  local dest="${MODULE_DIR}/src/main/resources/registration-processor-abis.json"
  if [[ ! -f "$dest" && -f "$sample" ]]; then
    cp "$sample" "$dest"
...
cmd_start() {
  ...
  check_prereqs
  if is_running; then
    echo "mock-abis already running (pid $(cat "$PID_FILE")) on port ${PORT}"
    print_endpoints
    return 0
  fi
  rm -f "$LOG_FILE"
  local jar
  jar="$(find_boot_jar)"
  echo "==> starting mock-abis from $(basename "$jar")"
...
wait_ready() {
  ...
  while [[ "$elapsed" -lt "$timeout" ]]; do
    ...
    if grep -q "Started ProxyAbisApplication" "$LOG_FILE" 2>/dev/null; then
...
cmd_stop() {
  ...
  local pid
  pid="$(cat "$PID_FILE")"
  echo "==> stopping mock-abis (${pid}) on port ${PORT}"
...
cmd_smoke() {
  ...
  local timeout="${1:-60}"
  local elapsed=0
  echo "==> waiting for mock-abis (up to ${timeout}s)"
  while [[ "$elapsed" -lt "$timeout" ]]; do
...
cmd_docker() {
  cmd_init
  need_cmd docker
  docker rm -f mock-abis >/dev/null 2>&1 || true
  echo "==> docker build ${IMAGE}"
  docker build -t "${IMAGE}" "${MODULE_DIR}"
  print_endpoints
  # shellcheck disable=SC2046
  exec docker run --rm -p "${PORT}:8081" --name mock-abis \
    $(docker_host_args) \
